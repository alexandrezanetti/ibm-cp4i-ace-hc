...
while (oc get routes --all-namespaces | grep -i "${PROJECT}-mq-qmgr-start-ibm-mq-qm"); do sleep 1 ; done
oc get routes --all-namespaces | grep "${PROJECT}-mq-qmgr" > /tmp/url-console-mq.txt
